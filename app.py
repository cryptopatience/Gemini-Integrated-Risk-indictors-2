...
def main():
    ...
    if GEMINI_AVAILABLE:
        ...
        with col_ai1:
            st.info("💡 Gemini 2.5 Flash가 현재 시장 상황을 분석하여 핵심 인사이트를 제공합니다.")
        
        with col_ai2:
            auto_analysis = st.checkbox("자동 분석", value=False, help="체크하면 페이지 로드 시 자동으로 AI 분석 실행")
        
        if auto_analysis or st.button("🚀 AI 분석 실행", type="primary", key="main_ai_analysis"):
            if 'main_ai_analysis' not in st.session_state or not auto_analysis:
        ...
        if 'main_ai_analysis' in st.session_state:
            ...
            with col1:
                ...
                st.info(st.session_state['main_ai_analysis'].get('market_status', '분석 중...'))
            
            with col2:
                st.markdown("**⚠️ 주요 리스크**")
                st.warning(st.session_state['main_ai_analysis'].get('key_risks', '분석 중...'))
            
            with col3:
                st.markdown("**💡 투자 전략**")
                st.success(st.session_state['main_ai_analysis'].get('strategy', '분석 중...'))
            
            with st.expander("📖 상세 AI 분석 보기", expanded=False):
                st.markdown(st.session_state['main_ai_analysis'].get('full_analysis', ''))
        ...
        if analysis_mode == "종합 분석":
            if st.button("🚀 종합 AI 분석 실행", type="primary"):
                with st.spinner("🧠 Gemini 분석 중..."):
                    analysis = generate_comprehensive_analysis(df, risk)
            ...
            if st.button("🔍 지표 분석 실행", type="primary"):
    ...
    st.markdown(
        ...
        <div style='text-align: center; color: gray; padding: 20px;'>
            ...
            <p>데이터 출처: FRED | AI: Gemini 2.5 Flash</p>
